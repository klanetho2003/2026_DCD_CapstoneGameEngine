...
public class InteractionManager
{
    ...
    /// </summary>
    private readonly List<NpcInteractionComponent>[] _byTrigger = new List<NpcInteractionComponent>[(int)ETriggerType.Count];

    // 디스패치 도중 Unregister(효과가 NPC를 디스폰) → 순회 안전을 위해 보류
    private bool _isDispatching;
    private readonly List<NpcInteractionComponent> _pendingUnregister = new();

    public void Init()
    {
        for (int i = 0; i < _byTrigger.Length; i++)
            _byTrigger[i] = new List<NpcInteractionComponent>(64);
    }
    ...
    #region 등록
    public void Register(NpcInteractionComponent component)
    {
        ...
        for (int t = 0; t < _byTrigger.Length; t++) // 모든 트리거 타입(ETriggerType)을 순회
        {
            ...
            if ((mask & (1 << t)) == 0)
                continue;
            List<NpcInteractionComponent> list = _byTrigger[t]; // 참조형
    ...
    public void Unregister(NpcInteractionComponent component)
    {
        if (_isDispatching)
        {
    ...
    {
        List<NpcInteractionComponent> list = _byTrigger[(int)trigger];
        if (list.Count == 0)
            return;
