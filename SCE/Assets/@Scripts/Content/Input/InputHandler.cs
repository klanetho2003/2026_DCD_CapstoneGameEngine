...
///   3. 상태 전환은 invoker.StateMachine.SetState()로 요청한다.
/// </summary>
public abstract class BaseInputHandler
{
    ...
    public virtual void OnSkillEventInvoke(IAnimationEventInvoker invoker, ESkillSlot slot) { }
...
public class MovementHandler : BaseInputHandler
{
    ...
    public override void OnSkillEventInvoke(IAnimationEventInvoker invoker, ESkillSlot slot)
    {
        /* To Do
         * slot에서 skill 찾기 -> skill 내부에 animation StateType과 어떤 Animation을 틀어줘야하는지를 enum으로 캐싱
         * 하나의 State를 쓰고, blend tree 내부에서 enum값을 int로 cast해서 분기하자
         * ex. 근접 공격(ESkillType.Melee = 1)을 int로 casting해서 SetInt(값)한 뒤에
        ...
        if (vAnim.AnimationHash.TryGetValue((int)EUserbleAnimState.Attack, out var hash) == false)
            return;


        var villager = vAnim.GetOwnerAsVillger;
        var id = villager.SkillSlots.GetSkillID(slot);
        villager.SkillBook.TryUseSkill(id);
    }
}
#endregion
